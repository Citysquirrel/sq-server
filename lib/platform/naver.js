...
const GetLive = async (channelId) => {
	...
	try {
		return new Promise((resolve) => {
			let data = [];
			axios.get(chzzkLiveAPI(channelId)).then((res) => {
				console.log(res);
			});
...
async function getChzzkStream(streamer) {
	try {
		const call = await GetLive(STREAMERS[streamer].chzzk.channelId);
		console.log(call);
